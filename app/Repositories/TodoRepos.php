...
class TodoRepos
{
    ...
    public function getTodoListsDetails($conditions = [])
    {
        ...
        if (!empty($conditions['start_date'])) {
            $todoListsModel = $todoListsModel->where('created_at', ">=", $conditions['start_date']);
        }

        if (!empty($conditions['end_date'])) {
            $endData = Carbon::createFromTimestamp(strtotime($conditions['end_date']))->addDay()->toDateString();
            $todoListsModel = $todoListsModel->where('created_at', "<=", $endData);
        }
